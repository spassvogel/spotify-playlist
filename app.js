...
const clientId = '76d82b549eb844b69f7b82c542cdd90f'; // your clientId
...
if (currentToken.access_token) {
  ...
  const parseTrack = (trackData) => {
    return `| ${parseTrackName(trackData)} | ${parseArtists(trackData)} |`
  }

  const parseTrackName = (trackData) => {
    return `[${trackData.track.name}](${trackData.track.external_urls.spotify})`
  }

  const parseArtists = (trackData) => {
    return trackData.track.artists.map(parseArtist).join(', ')
  }

  const parseArtist = (artistData) => {
...
async function getSavedTracks() {
  ...
  const fetchPage = async (url) => {
    ...
    if (json.next) {
      await fetchPage(json.next)
    }
  }

  await fetchPage('https://api.spotify.com/v1/me/tracks?limit=50&offset=0')
  return items
}
